...
public final class User implements UserDetails, Comparable<User> {
    ...
    public Set<Note> loadReadableNotes(JdbcTemplate jdbcTemplate) {
        ...
        return io.vavr.collection.HashSet.ofAll(jdbcTemplate.query(sql, ps -> { ps.setString(1, username);},(rs, rowNum) -> 
            new Note(
                ...
                Note.loadUserRoles(jdbcTemplate, UUID.fromString(rs.getString("id")))
            )
        ));
    }
